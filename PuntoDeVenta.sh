...
EOF
	...
	function Mostrar_base {
		...
		read Mod
		echo "cual tabla desea visualizar: "
		read Tab
		sqlite3 $Mod ".mode column" "SELECT * FROM $Tab;"

	}

	function Agregar {

		echo "Cual es la base de datos en la que deseas agregar mas datos: "
		read Base
		clear
		echo "Producto:"     
		read Producto      
...
EOF
	...
	function Inversion {
		read -p "De cual base de datos desea ver el total de su inversion: " opt
		sqlite3 $opt "SELECT SUM(Inversion) AS Total_inversion FROM Inventario;"
		

	}
	function Modificar {
		read -p "Cual es la base e datos que deseas modificar: " opt
		read -p "Cual es el nombre de la tabla que deseas modificar: " opti
		...
		sqlite3 $opt "UPDATE $opti SET $optio='$options' WHERE ID=$option"
	...
	function Dia_laboral {
		...
		echo -e "Para seguir con anotando las ventas del dia laboral presiona 1.\nSi deseas salir presiona 2."
		read -p "Has una eleccion: " optio
		case $optio in
			"1") continuar ;;
			"2") exit ;;
		esac
